...
- `repository-scan`: runs Trivy against the repository for dependency,
  secret, and IaC misconfiguration findings, then uploads SARIF to GitHub code
  scanning.
- `image-scan`: builds the local Gateway image, scans it with Trivy, uploads
  SARIF, and publishes a CycloneDX SBOM artifact.
...
- Treat SARIF findings as triage input, not as a replacement for dependency
  ownership.
